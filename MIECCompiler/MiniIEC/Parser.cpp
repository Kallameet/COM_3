...
void Parser::VarDecl() {
		Expect(11);
		Expect(1);
		std::wstring wsIdent(t->val); std::string stringIdent(wsIdent.begin(), wsIdent.end()); 
		Expect(5);
		Expect(3);
		BaseType* intType = new BaseType(BaseTypeName::INT); 
		Expect(4);
		_symTab.Add(_symFactory.CreateVarSymbol(stringIdent, intType, intType->GetSize())); 
		while (la->kind == 1) {
			Get();
			wsIdent = std::wstring(t->val); stringIdent = std::string(wsIdent.begin(), wsIdent.end()); 
			Expect(5);
			Expect(3);
			intType = new BaseType(BaseTypeName::INT); 
			Expect(4);
			_symTab.Add(_symFactory.CreateVarSymbol(stringIdent, intType, intType->GetSize())); 
		}
		Expect(12);
