...
void Scanner::Init() {
	...
	keywords.set(L"Integer", 3);
	keywords.set(L"PROGRAM", 8);
	keywords.set(L"BEGIN", 9);
...
Token* Scanner::NextToken() {
	...
	switch (state) {
		...
		case 6:
			case_6:
			if (ch == L'V') {AddCh(); goto case_7;}
			else {goto case_0;}
		case 7:
			case_7:
			if (ch == L'A') {AddCh(); goto case_8;}
			else {goto case_0;}
		case 8:
			case_8:
		...
		case 10:
			case_10:
			if (ch == L'V') {AddCh(); goto case_11;}
			else {goto case_0;}
		case 11:
			case_11:
			if (ch == L'A') {AddCh(); goto case_12;}
			else {goto case_0;}
		case 12:
			case_12:
			if (ch == L'R') {AddCh(); goto case_13;}
			else {goto case_0;}
		case 13:
			case_13:
			{t->kind = 12; break;}
		...
		case 32:
			...
			recEnd = pos; recKind = 1;
			if ((ch >= L'0' && ch <= L'9') || (ch >= L'A' && ch <= L'Z') || (ch >= L'a' && ch <= L'z')) {AddCh(); goto case_1;}
			else if (ch == L'_') {AddCh(); goto case_10;}
			else {t->kind = 1; wchar_t *literal = coco_string_create(tval, 0, tlen); t->kind = keywords.get(literal, t->kind); coco_string_delete(literal); break;}
		case 33:
			case_33:
			recEnd = pos; recKind = 1;
		...
		case 34:
			...
			recEnd = pos; recKind = 1;
			if ((ch >= L'0' && ch <= L'9') || (ch >= L'A' && ch <= L'Z') || (ch >= L'a' && ch <= L'z')) {AddCh(); goto case_1;}
			else if (ch == L'_') {AddCh(); goto case_6;}
			else {t->kind = 1; wchar_t *literal = coco_string_create(tval, 0, tlen); t->kind = keywords.get(literal, t->kind); coco_string_delete(literal); break;}
